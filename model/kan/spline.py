...
def curve2coef(x_eval, y_eval, grid, k, device="cpu"):
    ...
    # print("its fucking police!!")
    # print(mat.shape, y_eval.shape)
    # no = torch.where(torch.all((mat == 0), dim=2))
    # no1 = torch.where(torch.all((mat == 0), dim=1))
    # print(no[0].shape, no1[0].shape)
    ...
    #print(torch.linalg.lstsq(mat.to(device), y_eval.unsqueeze(dim=2).to(device),
