...
class KAN(nn.Module):
    def __init__(self, G = 5, k = 3, width = [2, 5, 1], b = nn.SiLU(), default_gird_range = [-1, 1], device = "cpu") -> None:
        ...
        for i in range(len(width) - 1):
            one_layer = KANLayer(width[i], width[i + 1], k = k, num = G, base_fun= b, device = device)
    ...
    def initial_grid_from_other_model(self, model, x):
        ...
        for i in range(len(self.layer)):
            ...
            self.layer[i].initialize_grid_from_parent(model.layer[i], model.acts_value[i])
        for i in range(len(self.layer)): 
            spb = self.layer[i]
            spb_parent = model.layer[i]
    ...
    def update_grid_from_sample(self, x):
        self.forward(x)
        for i in range(len(self.layer)):
            self.layer[i].update_grid_from_samples(self.acts_value[i])
    ...
    def plot(self):
        ...
        for i in range(w):
            x = torch.linspace(-3, 3, 100).unsqueeze(1).repeat(1, self.layer[i].in_dim).to(torch.double)
            y = self.layer[i](x)[2]
            for j in range(h):
